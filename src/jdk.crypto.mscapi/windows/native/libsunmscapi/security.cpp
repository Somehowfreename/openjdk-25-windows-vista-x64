  ...
  (JNIEnv *env, jobject obj, jstring jCertStoreName, jint jCertStoreLocation)
{
    ...
    __try
    {
        ...
        while (pCertContext = ::CertEnumCertificatesInStore(hCertStore, pCertContext))
        {
            ...
            // XP's Crypt32 predates CNG and does not accept the NCRYPT flag.
            // Keep CNG enabled on Windows 7 while selecting CryptoAPI on XP.
            DWORD dwNCryptFlags = JdkNCryptAvailable()
                    ? CRYPT_ACQUIRE_ALLOW_NCRYPT_KEY_FLAG : 0;

            // First, probe it silently
            if (::CryptAcquireCertificatePrivateKey(pCertContext,
                    dwNCryptFlags | CRYPT_ACQUIRE_SILENT_FLAG, NULL,
            ...
            else
            {
                ...
                if (::CryptAcquireCertificatePrivateKey(pCertContext, dwNCryptFlags, NULL,
                        &hCryptProv, &dwKeySpec, &bCallerFreeProv) == FALSE)
                {
                    PP("bHasNoPrivateKey = TRUE!!");
                    bHasNoPrivateKey = TRUE;
