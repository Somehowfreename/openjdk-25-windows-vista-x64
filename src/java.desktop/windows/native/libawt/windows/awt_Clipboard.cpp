...
volatile BOOL AwtClipboard::isModernClipboardListener = FALSE;
HWND AwtClipboard::nextClipboardViewer = NULL;

typedef BOOL (WINAPI *AddClipboardFormatListener_t)(HWND);
typedef BOOL (WINAPI *RemoveClipboardFormatListener_t)(HWND);
typedef BOOL (WINAPI *GetUpdatedClipboardFormats_t)(PUINT, UINT, PUINT);

static FARPROC GetUser32Proc(const char *name) {
    HMODULE user32 = ::GetModuleHandle(TEXT("user32.dll"));
    return user32 == NULL ? NULL : ::GetProcAddress(user32, name);
}
...
void AwtClipboard::WmDrawClipboard(JNIEnv *env) {
    WmClipboardUpdate(env);
    if (nextClipboardViewer != NULL) {
        ::SendMessage(nextClipboardViewer, WM_DRAWCLIPBOARD, 0, 0);
    }
}

void AwtClipboard::WmChangeCbChain(HWND removed, HWND next) {
    if (removed == nextClipboardViewer) {
        nextClipboardViewer = next;
    } else if (nextClipboardViewer != NULL) {
        ::SendMessage(nextClipboardViewer, WM_CHANGECBCHAIN,
                      (WPARAM)removed, (LPARAM)next);
    }
}
...
void AwtClipboard::RegisterClipboardViewer(JNIEnv *env, jobject jclipboard) {
    ...
    HWND toolkitWindow = AwtToolkit::GetInstance().GetHWnd();
    AddClipboardFormatListener_t addClipboardFormatListener =
            (AddClipboardFormatListener_t)
            GetUser32Proc("AddClipboardFormatListener");

    if (addClipboardFormatListener != NULL &&
            addClipboardFormatListener(toolkitWindow)) {
        isModernClipboardListener = TRUE;
        isClipboardViewerRegistered = TRUE;
    } else {
        /* Windows XP/Server 2003 clipboard-viewer-chain fallback. */
        nextClipboardViewer = ::SetClipboardViewer(toolkitWindow);
        isModernClipboardListener = FALSE;
        isClipboardViewerRegistered = TRUE;
    }
}
...
void AwtClipboard::UnregisterClipboardViewer(JNIEnv *env) {
    ...
    if (isClipboardViewerRegistered) {
        HWND toolkitWindow = AwtToolkit::GetInstance().GetHWnd();
        if (isModernClipboardListener) {
            RemoveClipboardFormatListener_t removeClipboardFormatListener =
                    (RemoveClipboardFormatListener_t)
                    GetUser32Proc("RemoveClipboardFormatListener");
            if (removeClipboardFormatListener != NULL) {
                removeClipboardFormatListener(toolkitWindow);
            }
        } else {
            ::ChangeClipboardChain(toolkitWindow, nextClipboardViewer);
    ...
    (JNIEnv *env, jobject self)
{
    ...
    GetUpdatedClipboardFormats_t getUpdatedClipboardFormats =
            (GetUpdatedClipboardFormats_t)
            GetUser32Proc("GetUpdatedClipboardFormats");
    if (getUpdatedClipboardFormats != NULL) {
        VERIFY(getUpdatedClipboardFormats(lpuiFormats, cFormats,
                                          &pcFormatsOut));
    } else {
        /*
         * Windows XP/Server 2003 exposes the current list by enumeration,
         * which (unlike GetUpdatedClipboardFormats) requires the clipboard
         * to be open while the formats are read.
         */
        HWND toolkitWindow = AwtToolkit::GetInstance().GetHWnd();
        BOOL closeClipboard = FALSE;
        HWND openClipboardWindow = ::GetOpenClipboardWindow();
        if (openClipboardWindow == NULL) {
            closeClipboard = ::OpenClipboard(toolkitWindow);
            openClipboardWindow = closeClipboard ? toolkitWindow : NULL;
        }
        if (openClipboardWindow == toolkitWindow) {
            UINT format = 0;
            while (pcFormatsOut < cFormats &&
                    (format = ::EnumClipboardFormats(format)) != 0) {
                lpuiFormats[pcFormatsOut++] = format;
            }
        }
        if (closeClipboard) {
            VERIFY(::CloseClipboard());
