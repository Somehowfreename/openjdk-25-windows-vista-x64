...
#ifdef _WIN32
/*
 * inet_pton and if_nametoindex were added to Windows after XP/Server 2003.
 * Resolve the native implementations when present, retaining the normal path
 * on Vista and newer, and use APIs available on XP for the fallback paths.
 */
static int
jdkxpInetPton(int family, const char *source, void *destination) {
    typedef INT (WSAAPI *InetPtonAFn)(INT, PCSTR, PVOID);
    static InetPtonAFn nativeFunction;
    static int resolved;
    ...
    if (!resolved) {
        ...
        nativeFunction = winsock == NULL ? NULL :
                (InetPtonAFn)GetProcAddress(winsock, "InetPtonA");
        resolved = 1;
    }
    if (nativeFunction != NULL) {
        return nativeFunction(family, source, destination);
    }
    ...
    memcpy(text, source, textLength + 1);
    memset(&address, 0, sizeof(address));
    addressLength = sizeof(address);
    if (WSAStringToAddressA(text, family, NULL,
                            (struct sockaddr *)&address,
                            &addressLength) == SOCKET_ERROR) {
        return WSAGetLastError() == WSAEAFNOSUPPORT ? -1 : 0;
    }
    if (family == AF_INET) {
        memcpy(destination,
...
jdkxpIfNameToIndex(const char *name) {
    typedef NET_IFINDEX (WINAPI *IfNameToIndexFn)(PCSTR);
    static IfNameToIndexFn nativeFunction;
    static int resolved;
    ...
    WCHAR wideName[IF_MAX_STRING_SIZE + 1];
    int hasWideName;

    if (!resolved) {
        HMODULE ipHelper = GetModuleHandleW(L"IPHLPAPI.DLL");
        nativeFunction = ipHelper == NULL ? NULL :
                (IfNameToIndexFn)GetProcAddress(ipHelper, "if_nametoindex");
        resolved = 1;
    }
    if (nativeFunction != NULL) {
    ...
    hasWideName = MultiByteToWideChar(CP_ACP, 0, name, -1,
                                      wideName, IF_MAX_STRING_SIZE + 1) != 0;
    ...
    if (status == NO_ERROR) {
        for (current = addresses; current != NULL; current = current->Next) {
            int adapterNameMatches = current->AdapterName != NULL &&
                    _stricmp(current->AdapterName, name) == 0;
            int friendlyNameMatches = hasWideName &&
                    current->FriendlyName != NULL &&
                    _wcsicmp(current->FriendlyName, wideName) == 0;
            if (adapterNameMatches || friendlyNameMatches) {
                result = current->Ipv6IfIndex != 0
                        ? current->Ipv6IfIndex : current->IfIndex;
                break;
            }
        }
    }
...
#else
#define jdkxpInetPton inet_pton
#define jdkxpIfNameToIndex if_nametoindex
#endif
...
static long parseScopeId(const char *str) {
    ...
    unsigned long scopeId = jdkxpIfNameToIndex(str);
    if (scopeId == 0) {
...
parseAllowedAddr(const char *buffer, struct in6_addr *result, int *isIPv4) {
    ...
    if (jdkxpInetPton(AF_INET6, buffer, &addr6) == 1) {
        *isIPv4 = 0;
    } else if (jdkxpInetPton(AF_INET, buffer, &addr) == 1) {
                               ...
                               char** actualAddress)
{
    ...
    if (!allowOnlyIPv4) {
        jdkxpInetPton(AF_INET6, "::ffff:0.0.0.0", &mappedAny);
