...
#ifdef _WIN32
    // The XP-compatible launcher maps a second jvm.dll from <jdk>\bin during
    // process startup. The canonical VM and its CDS archives remain in
    // <jdk>\bin\server on every supported Windows version.
    jio_snprintf(jvm_path, sizeof(jvm_path), "%s%sbin%sserver%sjvm.dll",
                 Arguments::get_java_home(), os::file_separator(),
                 os::file_separator(), os::file_separator());
#else
