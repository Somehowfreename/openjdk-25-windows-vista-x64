...
#ifdef JDKXP_PRELOAD_JVM
        /*
         * The launcher imports jvm.dll so XP allocates its static TLS during
         * process startup.  Reuse that module rather than loading the copy in
         * bin\\<variant> a second time.
         */
        handle = GetModuleHandle(JVM_DLL);
        if (handle == 0)
#endif
        {
