...
getHomeFromShell32()
{
    typedef HRESULT (WINAPI *SHGetKnownFolderPathFn)(
    ...
    if (u_path == NULL) {
        ...
        SHGetKnownFolderPathFn getKnownFolderPath = NULL;

        if (shell32 != NULL) {
            getKnownFolderPath = (SHGetKnownFolderPathFn)
                    GetProcAddress(shell32, "SHGetKnownFolderPath");
        }
        if (getKnownFolderPath != NULL) {
            hr = getKnownFolderPath(&FOLDERID_Profile, KF_FLAG_DONT_VERIFY,
                                    NULL, &tmpPath);
        } else {
            /*
             * SHGetKnownFolderPath was introduced in Vista.  XP and Server
             * 2003 expose the equivalent profile directory through the
             * older CSIDL API.  Resolve it dynamically as well so java.dll
             * does not acquire a loader-time dependency on a newer Shell32.
             */
            HMODULE folderModule = shell32;
            SHGetFolderPathWFn getFolderPath = NULL;
            WCHAR path[MAX_PATH];

            if (folderModule != NULL) {
                getFolderPath = (SHGetFolderPathWFn)
                        GetProcAddress(folderModule, "SHGetFolderPathW");
            }
            if (getFolderPath == NULL) {
                folderModule = LoadLibraryW(L"shfolder.dll");
                if (folderModule != NULL) {
                    getFolderPath = (SHGetFolderPathWFn)
                            GetProcAddress(folderModule, "SHGetFolderPathW");
                }
            ...
            if (getFolderPath != NULL) {
                hr = getFolderPath(NULL,
                                   CSIDL_PROFILE | CSIDL_FLAG_DONT_VERIFY,
                                   NULL, SHGFP_TYPE_CURRENT, path);
                if (SUCCEEDED(hr)) {
                    SIZE_T bytes = (wcslen(path) + 1) * sizeof(WCHAR);
                    tmpPath = (WCHAR*)CoTaskMemAlloc(bytes);
                    if (tmpPath == NULL) {
                        hr = E_OUTOFMEMORY;
                    } else {
                        memcpy(tmpPath, path, bytes);
                    }
                }
            }
        }
...
GetJavaProperties(JNIEnv* env)
{
    ...
    /* OS properties */
    {
         ...
         * Windows 2000                 5               0
         * Windows XP                   5               1
         * Windows XP Professional x64  5               2  (VER_NT_WORKSTATION)
         * Windows Server 2003          5               2  (!VER_NT_WORKSTATION)
