...
typedef HINTERNET (WINAPI *WinHttpOpenFn)(
        LPCWSTR, DWORD, LPCWSTR, LPCWSTR, DWORD);
typedef BOOL (WINAPI *WinHttpGetIEProxyConfigForCurrentUserFn)(
        WINHTTP_CURRENT_USER_IE_PROXY_CONFIG *);
typedef BOOL (WINAPI *WinHttpGetProxyForUrlFn)(
        HINTERNET, LPCWSTR, WINHTTP_AUTOPROXY_OPTIONS *, WINHTTP_PROXY_INFO *);

static WinHttpOpenFn jdkWinHttpOpen = NULL;
static WinHttpGetIEProxyConfigForCurrentUserFn
        jdkWinHttpGetIEProxyConfigForCurrentUser = NULL;
static WinHttpGetProxyForUrlFn jdkWinHttpGetProxyForUrl = NULL;

/*
 * WinHTTP is present on Windows 7 but is not guaranteed to be installed on
 * XP x64. Resolve it at runtime so newer Windows keeps the native system-proxy
 * implementation while XP can fall back to a direct connection.
 */
static BOOL loadWinHttp(void) {
    ...
    jdkWinHttpOpen = (WinHttpOpenFn)GetProcAddress(module, "WinHttpOpen");
    jdkWinHttpGetIEProxyConfigForCurrentUser =
            (WinHttpGetIEProxyConfigForCurrentUserFn)GetProcAddress(
                    module, "WinHttpGetIEProxyConfigForCurrentUser");
    jdkWinHttpGetProxyForUrl =
            (WinHttpGetProxyForUrlFn)GetProcAddress(
                    module, "WinHttpGetProxyForUrl");

    if (jdkWinHttpOpen == NULL ||
            jdkWinHttpGetIEProxyConfigForCurrentUser == NULL ||
            jdkWinHttpGetProxyForUrl == NULL) {
        FreeLibrary(module);
        jdkWinHttpOpen = NULL;
        jdkWinHttpGetIEProxyConfigForCurrentUser = NULL;
        jdkWinHttpGetProxyForUrl = NULL;
...
Java_sun_net_spi_DefaultProxySelector_init(JNIEnv *env, jclass clazz) {
    ...
    session = jdkWinHttpOpen(
            L"Only used internal", /* we need no real agent string here */
                                                       ...
                                                       jstring host)
{
    ...
    if (jdkWinHttpGetIEProxyConfigForCurrentUser(&ie_proxy_config) == FALSE) {
        /* cleanup and exit */
        (*env)->ReleaseStringChars(env, host, lpHost);
    ...
    if (use_auto_proxy) {
        ...
        use_auto_proxy = jdkWinHttpGetProxyForUrl(
                session, &url[0], &auto_proxy_options, &proxy_info);
        if (use_auto_proxy) {
