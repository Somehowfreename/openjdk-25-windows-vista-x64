...
#ifdef _WIN32
/*
 * Windows versions before Vista do not allocate static TLS for a DLL first
 * loaded with LoadLibrary.  Every JDK launcher therefore imports one harmless
 * JVM entry point, causing jvm.dll to be mapped by the process loader before
 * the initial thread starts.
 */
#ifdef _WIN64
#pragma comment(linker, "/include:__imp_JNI_GetDefaultJavaVMInitArgs")
#else
#pragma comment(linker, "/include:__imp__JNI_GetDefaultJavaVMInitArgs@4")
#endif
#endif
...
#endif /* WIN32 */
#ifdef MINECRAFT_LEGACY_WRAPPER
    {
        extern void minecraft_legacy_inject_options(int*, char***);
        minecraft_legacy_inject_options(&margc, &margv);
    }
#endif
